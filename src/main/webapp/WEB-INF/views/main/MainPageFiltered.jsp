...
<title>Filtered Products | KREAM</title>
		...
		<div class="sidebar">
			<div class="sidebar_container">
				...
				<form action="${pageContext.request.contextPath}/main/mainpageFiltered" method="get">
					...
					<button id = "filter" type="submit">필터 적용</button>
				</form>
			</div>
		</div>
		...
		<div id="paging">
			<ul>
				...
				<c:if test="${requestScope.pMap.prev}">
					<li><a href="${pageContext.request.contextPath}/main/mainpageFiltered?crtpage=${requestScope.pMap.startPageBtnNo}">◀</a></li>
				</c:if>

				<!-- Page number buttons -->
				<c:forEach begin="${requestScope.pMap.startPageBtnNo}" end="${requestScope.pMap.endPageBtnNo}" var="page">
					<li><a href="${pageContext.request.contextPath}/main/mainpageFiltered?crtpage=${page}">${page}</a></li>
				</c:forEach>

				<!-- Next button -->
				<c:if test="${requestScope.pMap.next}">
					<li><a href="${pageContext.request.contextPath}/main/mainpageFiltered?crtpage=${requestScope.pMap.endPageBtnNo}">▶</a></li>
				</c:if>
			</ul>
		</div>
	</div>
